import { DoubleLinkedNode } from "./d-linked-node";
...
export class DoubleLinkedList<T> {
  ...
  headNode(): DoubleLinkedNode<T> | null {
    return this.head;
  }
  ...
  tailNode(): DoubleLinkedNode<T> | null {
    return this.tail;
  }
  ...
  getIndexOfNode(node: DoubleLinkedNode<T>): number {
    let currentNode = this.head;
    let currentIndex = 0;
    while (currentNode !== null) {
      if (currentNode === node) {
  ...
  getAtIndex(index: number): DoubleLinkedNode<T> | undefined {
    if (index < 0 || index >= this._length) {
      return undefined;
    }
    let currentNode = this.head;
  ...
  getByValue(value: T): DoubleLinkedNode<T> | undefined {
    let currentNode = this.head;
    while (currentNode !== null) {
      if (currentNode.value === value) {
        return currentNode;
  ...
  removeNode(node: DoubleLinkedNode<T>): T | undefined {
    if (node === null) {
      return undefined;
    }
    if (node === this.head) {
  ...
  addNext(value: T, node: DoubleLinkedNode<T>): void {
    if (node === null) {
      return;
    }
  ...
  addPrev(value: T, node: DoubleLinkedNode<T>): void {
    const newNode = new DoubleLinkedNode(value);
    newNode.next = node;
  ...
  map<W>(callback: (value: T, index: number) => W): DoubleLinkedList<W> {
    const newList = new DoubleLinkedList<W>();
    let currentNode = this.head;
    let index = 0;
    while (currentNode !== null) {
